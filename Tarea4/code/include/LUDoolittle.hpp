...
namespace anpi {
  ...
  void luDoolittle(const Matrix<T>& A,
		...
		for (std::size_t i = 0 ; i < matrixSize ; i++) //Filling the permutation vector with the amount of columns of the matrix.
			permut[i] = T(i);

		anpi::Matrix<T> workMatrix = A;	//Copy of the original matrix to one that can be worked on.
		anpi::Matrix<T> factorMatrix(matrixSize,matrixSize);	//Matrix with the factors of the procedure.

		for (std::size_t i = 0 ; i < matrixSize-1; i++){ //Iterates through rows. Main for.
	
			std::size_t j = i; //It works diagonally.

			T pivot = workMatrix(i,j);	//The starting pivot always starts in an element of the diagonal.
			...
			for (std::size_t i_temp = i ; i_temp < matrixSize; i_temp++){
				...
				if(std::abs(workMatrix(i_temp,j)) > std::abs(pivot)){ //A biggest value is found.

					pivot = workMatrix(i_temp,j);	//Changes the pivot.
			...
			swapRows(workMatrix, i,row_pivot);	//Swaps the row with the old pivot with the row of the new one.

			for (std::size_t i2 = i ; i2 < matrixSize -1 ; i2++){ //Calculates the factors according to the row.

				T factor = workMatrix(i2+1,j)/workMatrix(i,j);	//Calculates the factor.

				for (std::size_t j2 = i ; j2 < matrixSize ; j2++) //Executes the changes in the U matrix with the right factor.

					workMatrix(i2+1,j2) = workMatrix(i2+1,j2)-workMatrix(i,j2)*factor;	//Executes the changes.

				factorMatrix(i2+1,j) = factor;	//Sets the factor calculated in the L matrix.
			}

		}

		LU = workMatrix + factorMatrix;  

		throw anpi::Exception("To be implemented yet");
  }

	template<class T> 
	void swapRows(Matrix<T>& matriz, std::size_t row1, std::size_t row2){

	/*
	 * Swap two rows given in the given matrix
	 */
